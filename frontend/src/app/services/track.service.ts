...
export class TrackService {
  ...
  updateCategoriesUrl: string =
    environment.apiUrl + '/api/tracks/updateCategory';
  trackNoPointsUrl: string = environment.apiUrl + '/api/tracks/noPoints';
  segmentMetadataUrl: string =
  ...
  constructor(
    ...
    private idbService: IdbcacheService,
  ) {}
  ...
  getTrackGeoJson(
    ...
    const trackResult = from(
      this.idbService.getTrackFeature(track.uuid).then((result) => {
        if (
          result.feature === null ||
          result.startTimestampMillis !== track.startTimestamp.toEpochMilli() ||
          result.endTimestampMillis !== track.endTimestamp.toEpochMilli()
        ) {
          console.log(
            'Track ' + track.uuid + ' not yet in indexeddb, retreiving',
          );
          ...
          console.log(
            'Successfully retreived feature for Track ' +
              track.uuid +
              ' from indexeddb',
          );
  ...
  getTrackGeoJsonFromUrl(id: string): Observable<Feature<GeoJSON.LineString>> {
    ...
    return this.http.get<Feature<GeoJSON.LineString>>(
      this.geoJsonUrl + '/' + id,
    );
  }
  ...
  getTrackCategories(): Promise<Array<string>> {
    return firstValueFrom(
      this.http.get(this.categoriesUrl) as Observable<Array<string>>,
    );
  }
  ...
  async getAllGroups(): Promise<Array<TrackGroup>> {
    return firstValueFrom(this.http.get<Array<TrackGroup>>(this.groupsUrl));
  }

  async createGroup(name: string): Promise<TrackGroup> {
    const params = new HttpParams().set('name', name);
    return firstValueFrom(
      this.http.post<TrackGroup>(this.groupsUrl, null, { params: params }),
    );
  }

  async getAllTracksWithMetadata(
    abortSignal: AbortSignal,
  ): Promise<Array<TrackMetadata>> {
    const tracksString = await fetch(this.tracksUrl, { signal: abortSignal });
    const text = await tracksString.text();
    const tracks = JSON.parse(text, (key, value) => {
      if (
    ...
    const test = sorted.map(async (track) => {
      ...
      if (metadata === null) {
        const trackObject = new TrackMetadata(
          track.uuid,
          track.startTimestamp,
          track.endTimestamp,
          track.category,
  ...
  async updateCategory(
    ...
    return await firstValueFrom(
      this.http.post(this.updateCategoriesUrl, null, {
        params: params,
        responseType: 'text',
      }),
    )
